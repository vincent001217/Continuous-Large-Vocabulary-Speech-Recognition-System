...
do
  ...
  if echo $realign_iters | grep -w $iter >/dev/null ; then
    echo "    aligning gmm training graphs equally"
  ...
  if [ ! -f $dir/$x.acc ]; then
    ...
    gmm-acc-stats-ali --binary=false $dir/$x.mdl ark,s,cs:$feat \
      ark,s,cs:$dir/train.ali $dir/$x.acc 2> $log
  else
  ...
  if [ ! -f $dir/$y.mdl ]; then
    ...
    echo "        log -> $log"
    gmm-est --binary=false --write-occs=$dir/$y.occs --mix-up=$numgauss \
