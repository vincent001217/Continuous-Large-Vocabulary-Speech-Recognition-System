...
lexicon_adapt=material/lexicon.adapt.txt
...
cat $lexicon_adapt | grep -v "<s>\|</s>" \
  > adapt/lexicon.txt

cat adapt/lexicon.txt | awk '{print $1}' | sort | uniq \
  | awk 'BEGIN{print "<eps> 0";} {printf("%s %d\n", $1, NR);} END{printf("#0 %d\n", NR+1);} ' \
  > adapt/words.txt

utility/make_lexicon_fst.pl adapt/lexicon.txt 0.5 sil \
  | fstcompile --isymbols=train/phones.txt \
    --osymbols=adapt/words.txt --keep_isymbols=false \
    --keep_osymbols=false \
  | fstarcsort --sort_type=olabel > adapt/L.fst
